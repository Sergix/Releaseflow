...
function match(x) {
    ...
    for (let i = 0; i < x.length; i++) {
        ...
        data[data.length] = (!x[i].trim().startsWith('-') ? '- ' : '') + (config_1.rfconfig.changelog.replace_links ? util.replacer(x[i], { 'links': true }) : x[i]);
    }
    return data;
}
exports.match = match;
function release(file = true) {
    ...
    let isDir = false;
    let filename = util.replacer(config_1.rfconfig.changelog.dist, { interpolate: true });
    ...
    if (isDir) {
        ...
        filename += 'changelog-' +
            (config_1.projectPackage.version !== undefined && config_1.projectPackage.version !== null ?
                config_1.projectPackage.version :
