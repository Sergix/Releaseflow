...
function release() {
    ...
    config_1.rfconfig.source.compression.forEach((type) => {
        if (type === 'zip') {
            filename = util.replacer(config_1.rfconfig.source.dist, { interpolate: true, ext: 'zip' });
            ...
            filename = util.replacer(config_1.rfconfig.source.dist, { interpolate: true, ext: 'tar.gz' });
